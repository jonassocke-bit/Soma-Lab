...
{const d=SAMMY_MEASURE_DEFS.find(x=>x.id==="hip_breadth");if(d){d.autoSearch="max";d.dynamicRule="upperPelvisWidth";d.implementation="Dynamische Maximum-Breite in einer oberen Becken-/Hüftzone zwischen Crotch und Omphalion; bewusst von der tieferen Buttock-Maximum-Ebene entkoppelt."}}
{const d=SAMMY_MEASURE_DEFS.find(x=>x.id==="chest_circumference");if(d){d.dynamicRule="femaleBustMaximum";d.implementation="Männlich: kalibrierte Brustebene. Weiblich: dynamische Maximum-Suche des horizontalen Umfangs innerhalb der Brustzone; Brustbreite und Brusttiefe übernehmen exakt dieselbe gefundene Ebene."}}
...
function sammyMeasureChestSliceV5(){
 const fixedY=sammyMeasureBaseY("chest")+sammyMeasureAnchorOffset("chest_circumference")/100,g=THREE.MathUtils.clamp(Number(annyParams.gender)||0,0,1),fixed=sammyMeasureTorsoSliceYV5(fixedY);if(g<=.30)return fixed;
 const waist=sammyMeasureSharedPlaneY("waist"),neck=sammyMeasureJoint("Neck1")||sammyMeasureJoint("Chest");if(!neck||!(neck[1]>waist))return fixed;const span=neck[1]-waist,lower=waist+span*.38,upper=waist+span*.78;let best=null;for(let i=0;i<32;i++){const y=lower+(upper-lower)*(i/31),q=sammyMeasureTorsoSliceYV5(y);if(q&&Number.isFinite(q.circ)&&(!best||q.circ>best.circ))best=q}if(!best)return fixed;
 // Smoothly move from the calibrated male plane into the female bust maximum so
 // intermediate gender morphs do not create an artificial measurement jump.
 if(g<.78){const t=(g-.30)/.48,y=fixedY+(best.plane-fixedY)*THREE.MathUtils.clamp(t,0,1);return sammyMeasureTorsoSliceYV5(y)}return best
}
function sammyMeasureHipBreadthV5(){
 ...
 // Hip breadth belongs to the upper pelvis/hip region, not the lower buttock
 // circumference maximum. Restrict the search so the line stays visibly above it.
 const lower=crotch+span*.42,upper=crotch+span*.80;if(!(upper>lower))return null;return sammySearchYSlicesV3(lower,upper,30,"width",false)
}
...
function sammyComputeMeasure(def,sectionCache=null){
 ...
 if(def.id==="neck_circumference")slice=sammyMeasureNeckSliceV3(false);
 ...
 else if(def.id==="hip_breadth")slice=sammyMeasureHipBreadthV5();
  ...
  const key="semantic:chest:v5";slice=sectionCache?.get(key);if(!slice){slice=sammyMeasureChestSliceV5();if(sectionCache&&slice)sectionCache.set(key,slice)}
 }
...
function sammyMeasureLandmarksV3(results){
 ...
 add("Buttock",pointOnSlice("buttock_circumference","back"),"buttock_circumference");const hs=results.hip_breadth?.line?.slice;if(hs)add("Hip level",sammySliceCenterWorldV3(hs),"hip_breadth");
...
// -----------------------------------------------------------------------------
// Sammy v0.8.1 CALIBRATION LAB R2
...
// -----------------------------------------------------------------------------
const SAMMY_CAL_DB="sammy-calibration-lab-v081",SAMMY_CAL_DB_VERSION=1,SAMMY_CAL_RUN_STORE="runs",SAMMY_CAL_RECORD_STORE="records";
...
function sammyCalLiveReadout(results,detail){const el=$("#sammyCalibrationLiveMeasures");if(!el)return;const ids=["stature","chest_circumference","waist_circumference","natural_waist_circumference","buttock_circumference","hip_breadth","upperarm_circumference","calf_circumference"];el.innerHTML=`<b>${escapeHtml(detail||"LIVE")}</b>`+ids.map(id=>{const d=SAMMY_MEASURE_DEFS.find(x=>x.id===id),v=results?.[id]?.valueCm;return `<span>${escapeHtml(d?.label||id)} <strong>${Number.isFinite(v)?v.toFixed(1):"—"} cm</strong></span>`}).join("")}
function sammyCalNewRun(mode=sammyCalibration.mode){const c=SAMMY_CAL_CONFIG[mode],refs=sammyCalReferences().slice(0,c.refs),sliders=sammyCalSliderDefs();return {schema:SAMMY_CAL_SCHEMA,runId:sammyCalRunId(),appVersion:"0.8.1",mode,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),stage:"reference",cursor:0,ordinal:0,references:refs,sliders,referenceMeasures:{},effects:{},relevantSliderIds:[],candidatePairs:[],deepInteractionPairs:[],screening:{thresholdCm:c.screenThresholdCm,byPair:{},screenedCount:0,strongCount:0},validation:{sumSq:{},count:{},overallSumSq:0,overallCount:0},notes:{measurementEngine:"v0.8.1 semantic MEAS; female dynamic bust + upper hip breadth",rawMorphUnits:"core 0..1 except age 0.70..1.00; logical local sliders -1..1",symmetry:"matching l-/r- Anny morphs are calibrated as one logical L+R slider",interactionStrategy:"all plausible pairs cheap-screened in Standard/Deep; only non-additive pairs receive deep scan",liveDefault:true}}}
...
function sammyMeasureInlineEditor(d,c){
 ...
 const dynamicFemale=d.dynamicRule==="femaleBustMaximum"&&sammyMeasureScope==="female",dynamicNote=d.dynamicRule==="femaleBustMaximum"?`<div class="sammyMeasureAutoRule"><b>Weiblich · dynamisches Brustmaximum</b><small>Bei weiblicher Kalibrierung sucht Sammy automatisch den größten horizontalen Umfang innerhalb der Brustzone. Männlich bleibt die kalibrierte Brustebene erhalten.</small></div>`:"";
