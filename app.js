...
function sammyComputeMeasure(def,sectionCache=null){
 ...
 else if(def.id==="upperarm_circumference")slice=sammySearchArmMaxV3(sammyMeasureJoint("RightArm"),sammyMeasureJoint("RightForeArm"),.42,.82,28);
...
// -----------------------------------------------------------------------------
// Sammy v0.8.0 CALIBRATION LAB
// Multi-stage, resumable, live-visible morph → measurement calibration.
// This is deliberately isolated inside MEAS; startup/Anny/SOMA/Rig are untouched.
// -----------------------------------------------------------------------------
const SAMMY_CAL_DB="sammy-calibration-lab-v080",SAMMY_CAL_DB_VERSION=1,SAMMY_CAL_RUN_STORE="runs",SAMMY_CAL_RECORD_STORE="records";
const SAMMY_CAL_SCHEMA="sammy-calibration-lab-v1";
const SAMMY_CAL_CONFIG={
 quick:{label:"Quick",refs:2,levels:3,maxPairs:28,interactionRefs:2,globalSamples:300,validationSamples:60,localPerGlobal:5,localAmp:.52,liveDelay:55},
 standard:{label:"Standard",refs:4,levels:5,maxPairs:120,interactionRefs:2,globalSamples:1500,validationSamples:220,localPerGlobal:9,localAmp:.68,liveDelay:60},
 deep:{label:"Deep",refs:6,levels:7,maxPairs:360,interactionRefs:4,globalSamples:5000,validationSamples:600,localPerGlobal:15,localAmp:.90,liveDelay:65}
};
...
function sammyCalSliderDefs(){
 ...
 const locals=(annyMeta?.local_change_labels||[]).map(key=>({id:`local:${key}`,key,label:prettyAnnyLabel(key),kind:"local",category:annyMeta?.local_change_categories?.[key]||"other",min:-1,max:1,unit:"raw"}));return [...core,...locals]
}
...
function sammyCalSliderValue(shape,d){return d.kind==="local"?Number(shape.local[d.key]||0):Number(shape.core[d.key]??.5)}
function sammyCalSetSlider(shape,d,value){if(d.kind==="local")shape.local[d.key]=Number(value);else shape.core[d.key]=Number(value);return shape}
...
function sammyCalDelay(ms){return new Promise(resolve=>setTimeout(resolve,ms))}
async function sammyCalShowFrame(results){
 sammyMeasureResultsCache=results;sammyMeasureOverlayMode="all";sammyMeasureLandmarksVisible=true;sammyMeasureLabelsVisible=false;sammyUpdateMeasureOverlay(results);
 for(const d of SAMMY_MEASURE_DEFS){const el=document.querySelector(`[data-measure-value="${d.id}"]`);if(el)el.textContent=sammyMeasureFormat(results[d.id]?.valueCm)}
 if(sammyCalibration.turbo){await new Promise(r=>setTimeout(r,0));return}await new Promise(r=>requestAnimationFrame(()=>setTimeout(r,SAMMY_CAL_CONFIG[sammyCalibration.run?.mode||sammyCalibration.mode].liveDelay)))
}
async function sammyCalApplyShape(shape){
...
function sammyCalStageLabel(stage){return ({reference:"0 · Referenzkörper",single:"1 · Slider Scan",analysis:"2 · Relevanzanalyse",interaction:"3 · Interaktionen",global:"4 · Global Sampling",validation:"5 · Validierung",complete:"6 · Fertig"})[stage]||stage}
function sammyCalStageTotal(run){const c=SAMMY_CAL_CONFIG[run.mode];if(run.stage==="reference")return run.references.length;if(run.stage==="single")return run.sliders.length*run.references.length*c.levels;if(run.stage==="analysis")return 1;if(run.stage==="interaction")return (run.candidatePairs?.length||0)*Math.min(c.interactionRefs,run.references.length)*4;if(run.stage==="global")return c.globalSamples;if(run.stage==="validation")return c.validationSamples;return 1}
function sammyCalStatus(detail=""){
 const run=sammyCalibration.run||sammyCalibration.lastRun,stage=run?.stage||"—",total=run?sammyCalStageTotal(run):0,cursor=run?.cursor||0,p=total?Math.min(1,cursor/total):0;const st=$("#sammyCalibrationStatus"),bar=$("#sammyCalibrationProgressBar"),cur=$("#sammyCalibrationCurrent");if(st)st.textContent=run?`${sammyCalStageLabel(stage)} · ${Math.min(cursor,total)} / ${total}${sammyCalibration.paused?" · PAUSE":""}`:"Noch kein Calibration-Lauf.";if(bar)bar.style.width=`${(p*100).toFixed(1)}%`;if(cur&&detail)cur.textContent=detail;
 const start=$("#sammyCalibrationStart"),pause=$("#sammyCalibrationPause"),reset=$("#sammyCalibrationReset");if(start)start.textContent=sammyCalibration.running?"Läuft …":(run&&run.stage!=="complete"?"Fortsetzen":"Neue Kalibrierung");if(start)start.disabled=sammyCalibration.running;if(pause)pause.disabled=!sammyCalibration.running;if(reset)reset.disabled=sammyCalibration.running
}
function sammyCalLiveReadout(results,detail){const el=$("#sammyCalibrationLiveMeasures");if(!el)return;const ids=["stature","chest_circumference","waist_circumference","natural_waist_circumference","buttock_circumference","upperarm_circumference","calf_circumference"];el.innerHTML=`<b>${escapeHtml(detail||"LIVE")}</b>`+ids.map(id=>{const d=SAMMY_MEASURE_DEFS.find(x=>x.id===id),v=results?.[id]?.valueCm;return `<span>${escapeHtml(d?.label||id)} <strong>${Number.isFinite(v)?v.toFixed(1):"—"} cm</strong></span>`}).join("")}
function sammyCalNewRun(mode=sammyCalibration.mode){const c=SAMMY_CAL_CONFIG[mode],refs=sammyCalReferences().slice(0,c.refs),sliders=sammyCalSliderDefs();return {schema:SAMMY_CAL_SCHEMA,runId:sammyCalRunId(),appVersion:"0.8.0",mode,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),stage:"reference",cursor:0,ordinal:0,references:refs,sliders,referenceMeasures:{},effects:{},relevantSliderIds:[],candidatePairs:[],validation:{sumSq:{},count:{},overallSumSq:0,overallCount:0},notes:{measurementEngine:"v0.8.0 semantic MEAS; cm output",rawMorphUnits:"core 0..1 except age 0.70..1.00; local -1..1",liveDefault:true}}}
function sammyCalEffectSlot(run,d){return run.effects[d.id]??={id:d.id,label:d.label,kind:d.kind,category:d.category,maxAbs:{},regression:{}}}
function sammyCalUpdateEffects(run,d,ref,shape,measureObj){const base=run.referenceMeasures[ref.id];if(!base)return;const x=sammyCalSliderValue(shape,d)-(d.kind==="local"?0:Number(ref.core[d.key]??.5)),slot=sammyCalEffectSlot(run,d);for(const m of SAMMY_MEASURE_DEFS){const y=(measureObj[m.id]??NaN)-(base[m.id]??NaN);if(!Number.isFinite(y))continue;slot.maxAbs[m.id]=Math.max(Number(slot.maxAbs[m.id]||0),Math.abs(y));if(Math.abs(x)>1e-8){const r=slot.regression[m.id]??={n:0,sx:0,sy:0,sxx:0,sxy:0};r.n++;r.sx+=x;r.sy+=y;r.sxx+=x*x;r.sxy+=x*y}}}
...
function sammyCalAnalyze(run){
 const threshold=.15,scored=[];for(const d of run.sliders){const e=run.effects[d.id],max=e?Math.max(0,...Object.values(e.maxAbs||{}).map(Number)):0;if(max>=threshold){run.relevantSliderIds.push(d.id);scored.push({id:d.id,max})}}
 run.relevantSliderIds=[...new Set(run.relevantSliderIds)];const rel=run.relevantSliderIds.map(id=>sammyCalDescriptor(run,id)).filter(Boolean),pairs=[];
 for(let i=0;i<rel.length;i++)for(let j=i+1;j<rel.length;j++){const a=run.effects[rel[i].id],b=run.effects[rel[j].id];let score=0,shared=0;for(const m of SAMMY_MEASURE_DEFS){const x=Number(a?.maxAbs?.[m.id]||0),y=Number(b?.maxAbs?.[m.id]||0);if(x>=threshold&&y>=threshold){shared++;score+=Math.min(x,y)}}if(shared)pairs.push({a:rel[i].id,b:rel[j].id,score:Number(score.toFixed(4)),shared})}
 pairs.sort((a,b)=>b.score-a.score);run.candidatePairs=pairs.slice(0,SAMMY_CAL_CONFIG[run.mode].maxPairs);run.analysis={thresholdCm:threshold,relevantCount:run.relevantSliderIds.length,totalSliders:run.sliders.length,pairCandidatesTotal:pairs.length,pairCandidatesTested:run.candidatePairs.length,topRelevant:scored.sort((a,b)=>b.max-a.max).slice(0,30)}
}
function sammyCalPairValue(d,hi){if(d.kind==="local")return hi?.68:-.68;if(d.key==="age")return hi?.94:.74;if(d.key==="gender")return hi?1:0;return d.min+(d.max-d.min)*(hi?.75:.25)}
function sammyCalPredictTwo(run,ref,a,b,shape){const base=run.referenceMeasures[ref.id],out={};for(const m of SAMMY_MEASURE_DEFS){let v=base?.[m.id];if(!Number.isFinite(v)){out[m.id]=null;continue}for(const d of [a,b])v+=sammyCalSlope(run,d.id,m.id)*(sammyCalSliderValue(shape,d)-(d.kind==="local"?0:Number(ref.core[d.key]??.5)));out[m.id]=Number(v.toFixed(4))}return out}
...
function sammyCalGlobalShape(run,index,validation=false){
 const sex=index%2,shape=sammyCalMakeShape({core:sammyCalCoreDefaults(sex)}),q=(d)=>sammyCalHalton(index+(validation?100003:0),d);shape.core.gender=sex;shape.core.age=SAMMY_ADULT_SHAPE_AGE_MIN+(SAMMY_ADULT_SHAPE_AGE_MAX-SAMMY_ADULT_SHAPE_AGE_MIN)*q(0);shape.core.height=.04+.92*q(1);shape.core.weight=.04+.92*q(2);shape.core.muscle=.04+.92*q(3);shape.core.proportions=.04+.92*q(4);shape.core.cupsize=sex?(.04+.92*q(5)):.5;shape.core.firmness=sex?(.08+.88*q(6)):.5;
 const relevant=run.relevantSliderIds.map(id=>sammyCalDescriptor(run,id)).filter(d=>d?.kind==="local"),cfg=SAMMY_CAL_CONFIG[run.mode],k=Math.min(cfg.localPerGlobal,relevant.length);if(relevant.length)for(let j=0;j<k;j++){const ix=(index*17+j*31)%relevant.length,d=relevant[ix],v=(q(12+j*2)*2-1)*cfg.localAmp;shape.local[d.key]=v}return shape
}
function sammyCalPredictGlobal(run,shape){const ref=shape.core.gender>=.5?sammyCalRef(run,"female_avg"):sammyCalRef(run,"male_avg"),base=run.referenceMeasures[ref?.id];if(!ref||!base)return null;const out={};for(const m of SAMMY_MEASURE_DEFS){let v=base[m.id];if(!Number.isFinite(v)){out[m.id]=null;continue}for(const id of run.relevantSliderIds){const d=sammyCalDescriptor(run,id);if(!d)continue;const delta=sammyCalSliderValue(shape,d)-(d.kind==="local"?0:Number(ref.core[d.key]??.5));if(Math.abs(delta)>1e-8)v+=sammyCalSlope(run,id,m.id)*delta}out[m.id]=Number(v.toFixed(4))}return out}
...
async function sammyCalStepSingle(run){const c=SAMMY_CAL_CONFIG[run.mode],perSlider=run.references.length*c.levels,si=Math.floor(run.cursor/perSlider),rem=run.cursor%perSlider,ri=Math.floor(rem/c.levels),li=rem%c.levels,d=run.sliders[si],ref=run.references[ri],levels=sammyCalLevels(d,c.levels),value=levels[li],shape=sammyCalSetSlider(sammyCalMakeShape(ref),d,value),results=await sammyCalApplyShape(shape),mo=sammyCalMeasureObject(results),detail=`${d.label} · ${value.toFixed(3)} · ${ref.label}`;sammyCalUpdateEffects(run,d,ref,shape,mo);sammyCalLiveReadout(results,detail);await sammyCalRecord(run,"single",run.cursor,detail,shape,results,{sliderId:d.id,value:Number(value.toFixed(6)),referenceId:ref.id});run.cursor++;if(run.cursor>=run.sliders.length*run.references.length*c.levels)sammyCalAdvance(run,"analysis")}
async function sammyCalStepAnalysis(run){sammyCalAnalyze(run);run.cursor=1;sammyCalStatus(`Relevante Slider: ${run.analysis.relevantCount}/${run.analysis.totalSliders} · Interaction-Kandidaten: ${run.analysis.pairCandidatesTested}`);await sammyCalPutRun(run);sammyCalAdvance(run,"interaction")}
async function sammyCalStepInteraction(run){const c=SAMMY_CAL_CONFIG[run.mode],refs=run.references.slice(0,Math.min(c.interactionRefs,run.references.length)),perPair=refs.length*4,pi=Math.floor(run.cursor/perPair),rem=run.cursor%perPair,ri=Math.floor(rem/4),combo=rem%4,pair=run.candidatePairs[pi],a=sammyCalDescriptor(run,pair.a),b=sammyCalDescriptor(run,pair.b),ref=refs[ri],shape=sammyCalMakeShape(ref),av=sammyCalPairValue(a,combo>=2),bv=sammyCalPairValue(b,combo%2===1);sammyCalSetSlider(shape,a,av);sammyCalSetSlider(shape,b,bv);const results=await sammyCalApplyShape(shape),actual=sammyCalMeasureObject(results),pred=sammyCalPredictTwo(run,ref,a,b,shape),residuals={};let maxResidual=0;for(const m of SAMMY_MEASURE_DEFS){if(Number.isFinite(actual[m.id])&&Number.isFinite(pred[m.id])){const e=actual[m.id]-pred[m.id];residuals[m.id]=Number(e.toFixed(4));maxResidual=Math.max(maxResidual,Math.abs(e))}}const detail=`${a.label} × ${b.label} · ${ref.label}`;sammyCalLiveReadout(results,detail);await sammyCalRecord(run,"interaction",run.cursor,detail,shape,results,{pair:{a:a.id,b:b.id,aValue:av,bValue:bv,score:pair.score},referenceId:ref.id,additiveResidualCm:residuals,maxResidualCm:Number(maxResidual.toFixed(4))});run.cursor++;if(run.cursor>=run.candidatePairs.length*refs.length*4)sammyCalAdvance(run,"global")}
async function sammyCalStepGlobal(run){const shape=sammyCalGlobalShape(run,run.cursor,false),results=await sammyCalApplyShape(shape),detail=`Global sample ${run.cursor+1}`;sammyCalLiveReadout(results,detail);await sammyCalRecord(run,"global",run.cursor,detail,shape,results,{sampling:"deterministic Halton core + sparse relevant-local subset"});run.cursor++;if(run.cursor>=SAMMY_CAL_CONFIG[run.mode].globalSamples)sammyCalAdvance(run,"validation")}
...
async function sammyCalRunner(){const run=sammyCalibration.run;if(!run)return;sammyCalibration.running=true;sammyCalibration.paused=false;sammyCalibration.cancelRequested=false;sammyCalStatus("Calibration startet …");try{while(sammyCalibration.running&&!sammyCalibration.paused&&!sammyCalibration.cancelRequested&&run.stage!=="complete"){
  if(run.stage==="reference")await sammyCalStepReference(run);else if(run.stage==="single")await sammyCalStepSingle(run);else if(run.stage==="analysis")await sammyCalStepAnalysis(run);else if(run.stage==="interaction"){if(!run.candidatePairs?.length)sammyCalAdvance(run,"global");else await sammyCalStepInteraction(run)}else if(run.stage==="global")await sammyCalStepGlobal(run);else if(run.stage==="validation")await sammyCalStepValidation(run);else throw new Error(`Unbekannte Calibration-Stufe ${run.stage}`);await sammyCalPutRun(run);sammyCalStatus();}
 if(run.stage==="complete"){sammyCalibration.running=false;sammyCalibration.lastRun=run;sammyCalStatus(`Fertig · additive Validierungs-RMSE ${run.validation?.overallRmseCm??"—"} cm`);const cur=$("#sammyCalibrationCurrent");if(cur)cur.textContent=`Calibration abgeschlossen · ${run.ordinal} Körper/Tests gespeichert · ${run.analysis?.relevantCount??0} relevante Slider · RMSE ${run.validation?.overallRmseCm??"—"} cm`;}
 }catch(e){console.error("Calibration Lab",e);sammyCalibration.running=false;sammyCalibration.paused=true;sammyCalStatus(`FEHLER: ${e?.message||e}`);sammyReportError?.(e,{source:"Calibration Lab"})}finally{sammyCalibration.running=false;sammyCalStatus()}}
async function sammyCalStartOrResume(){if(sammyCalibration.running)return;if(!annyPackLoaded){sammyCalStatus("Anny-Pack ist noch nicht bereit.");return}if(!sammyMeasureSession){sammyCalStatus("MEAS zuerst öffnen.");return}let run=sammyCalibration.run;if(!run||run.stage==="complete"){run=sammyCalNewRun(sammyCalibration.mode);sammyCalibration.run=run;await sammyCalPutRun(run)}sammyCalibration.paused=false;sammyCalibration.cancelRequested=false;sammyCalRunner()}
...
async function sammyCalExport(){const run=sammyCalibration.run||sammyCalibration.lastRun;if(!run){sammyCalStatus("Kein Calibration-Lauf zum Exportieren.");return}sammyCalStatus("JSON wird zusammengestellt …");const records=await sammyCalGetRecords(run.runId),payload={schema:SAMMY_CAL_SCHEMA,app:"Sammy",version:"0.8.0",generated:new Date().toISOString(),interpretation:{purpose:"Morph/Slider → Körpermaß-Kalibrierung für den späteren inversen Sammy Body Solver",rawSliderUnits:{core:"0..1; age/shapeAge 0.70..1.00",local:"-1..1",measurements:"cm"},stages:{reference:"Referenzkörper und Baseline-Maße",single:"Einzelslider über mehrere Stützstellen",analysis:"automatische Relevanz- und Paarselektion",interaction:"gezielte Sliderpaar-Tests",global:"mehrdimensionale Körperstichprobe",validation:"additives Basismodell gegen unbekannte Körper",complete:"Lauf abgeschlossen"},note:"ANSUR wird erst nach dieser Morph-Kalibrierung statistisch als Zielmaß-Schätzer ausgewertet; dieses JSON beschreibt ausschließlich Sammys eigene Morph→Maß-Abbildung."},run,measureDefinitions:SAMMY_MEASURE_DEFS.map(d=>({id:d.id,label:d.label,ansur:d.ansur,kind:d.kind,section:d.section||null,group:d.group||null,internal:!!d.internal,autoSearch:d.autoSearch||null})),measureCalibration:JSON.parse(JSON.stringify(sammyMeasureLoadCalibration())),records};const blob=new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`Sammy_Calibration_Lab_${run.mode}_${run.runId}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1500);sammyCalStatus(`Export · ${records.length} Records`)}
function sammyCalInitUI(){document.querySelectorAll("[data-cal-mode]").forEach(b=>b.onclick=()=>sammyCalSetMode(b.dataset.calMode));const start=$("#sammyCalibrationStart"),pause=$("#sammyCalibrationPause"),reset=$("#sammyCalibrationReset"),exp=$("#sammyCalibrationExport"),turbo=$("#sammyCalibrationTurbo");if(start)start.onclick=sammyCalStartOrResume;if(pause)pause.onclick=sammyCalPause;if(reset)reset.onclick=sammyCalReset;if(exp)exp.onclick=sammyCalExport;if(turbo)turbo.onclick=sammyCalToggleTurbo;sammyCalSetMode("standard");sammyCalLoadLatest()}
...
function sammyEnterMeasureMode(){
 ...
 sammyApplyMeasurementRelative(sammyMeasurementIdentityPose(),"Sammy Measurement T-Pose");sammyCameraTo("measure",900,false);sammyCalLoadLatest();const lb=$("#sammyMeasureLandmarks"),nb=$("#sammyMeasureLabels");if(lb){lb.classList.add("active");lb.textContent="Landmarks AN"}if(nb){nb.classList.remove("active");nb.textContent="Namen AUS"}sammyMeasureRefresh(true)
}
function sammyExitMeasureMode(instantCamera=false){
 const s=sammyMeasureSession;if(!s)return null;if(sammyCalibration?.running)sammyCalPause();sammyMeasureSession=null;sammyClearMeasureOverlay();
