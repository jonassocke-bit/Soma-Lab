...
// v0.6.5: exact browser-side Anny blendshape engine on canonical SOMA topology.
...
function parseNPY(u8){
 ...
 } else throw new Error("NPY dtype "+descr+" noch nicht unterstützt (v0.6.5 unterstützt f4/f8, i1/i2/i4/i8, u1/u2/u4 sowie U/S-Strings)");
...
async function loadCurrentRigPack(){
 try{
  ...
  // v0.6.5: the fresh v2 rig-pack stores REAL newlines, while the very first
  ...
  if(midMissing.length){if(asset.cacheHit)await assetCacheDelete(ASSET_KEY.currentRig);throw new Error("Rig-Pack ist noch v1/Low-only. Für v0.6.5 bitte den neuen ‘Build Anny SOMA Engine v3’-Workflow einmal ausführen; danach erneut laden.")}
...
Der aktuelle v0.2.x-Rig-Datenstand ist als kleiner Browser-Pack vorhanden. v0.6.5 kann daraus jetzt direkt den internen Expanded-/Twist-Pfad mit ${targetNames.length} Skinning-Joints aktivieren; die Bedienung bleibt bei den 77 öffentlichen Pose-Joints.`);
...
v0.6.5 versucht den Rig-Pack in dieser Reihenfolge:
...
// v0.6.5: exact structural/orientation contract extracted from the uploaded
...
// v0.6.5 semantic hand-chain correction.
...
function buildSammyMixamoBridgeScene(){
 ...
 // shoulders, arms and hands. v0.6.5 bakes the official SOMA T-pose into the
 ...
 // v0.6.5: dedicated thumb-plane refinement. The general shortest-arc frame
...
async function exportMixamoComparisonFrame(){
 ...
 try{
  ...
  const bytes=new mod.FBXExporter().parseSync(bridge.scene,{axisUp:"Y",axisForward:"-Z",unitScale:100,bakeSpaceTransform:false,includeAnimations:false,customProperties:true,creator:"Sammy Axis16 Retarget Comparison v0.6.5"});
...
async function exportSammyMixamoBridge(){
 ...
 try{
  ...
  const bytes=new mod.FBXExporter().parseSync(bridge.scene,{
   ...
   creator:"Sammy Mixamo XBotContract65 T-Pose Axis16 v0.6.5"
  });
...
Mesh und Skeleton sind in die offizielle SOMA T-Pose gebacken. v0.6.5: 52-Bone-Frame-Transport + Thumb-Plane-Fix + korrigierte SOMA→Mixamo-Fingersemantik (Metacarpal/MCP/Terminal).
...
v0.6.5 korrigiert genau den im iPhone-Test sichtbaren Grundhaltungsfehler:
...
function applyAnnyAxis16RetargetPose(rest,relative3,markMoved=true,report=true,label="Axis16 → exact Anny/SOMA"){
 ...
 // v0.6.5: world motion delta is applied to the TRANSPORTED NATIVE ANNY frame.
...
async function setDisplayLOD(lod){
 if(lod===displayLOD)return true;if(lod==="mid"){
  if(shapeEngine!=="anny"){info("#lodInfo","Mid ist in v0.6.5 bewusst für den Anny-Pfad aktiviert. Zuerst Anny verwenden.");return false}
...
async function startFullShapeAnalysis(){
 ...
 try{
  if(shapeEngine!=="soma-pca")throw new Error("Der alte 128-PC-Analyzer gilt nur für SOMA-PCA. Für v0.6.5 Anny direkt über die nativen Parameter testen.");
...
Wichtig: Umfang/Tiefe sind in v0.6.5 bewusst sichtbare Slice-Proxies. Die Mathematik des Modifiers wird damit real getestet; die endgültigen BODY-LAB-Messdefinitionen werden später gegen echte anthropometrische Landmarken/Messregeln validiert.`);
  updateDecision()
 }catch(e){
  console.error(e);
...
async function testAxisReferenceFixFromRepo(){
 try{
  ...
  stopPoseAnimation(false);
  const result=applyAnnyAxis16RetargetPose(currentRestMid,rel,true,false,"v0.6.5 Axis16-Referenz-FK");
...
v0.6.5 ändert erstmals NICHT die Animation und NICHT Anny-FK selbst, sondern den dazwischenliegenden Referenzvertrag:
...
Die statische Axis16-Bridge ist ab v0.6.5 der Bewegungs-Nullpunkt. Der animierte Mixamo-T-Pose-Clip wird nur noch als Kalibrier-/Kompatibilitätsprüfung gespeichert, damit Mixamos eigene Wrist-/Thumb-Pose nicht versehentlich aus jeder Animation herausgerechnet wird.`)
}
...
async function convertMixamoFbxMotion(arrayBuffer,filename="Mixamo FBX"){
 ...
 for(const [k,b] of bones){
  // v0.6.5: motion zero is the STATIC Axis16 bridge, not Mixamo's animated
...
Motion-Zero: STATISCHE Axis16-Bridge (v0.6.5)
...
`:""}Playback: ${userAnimFps} fps · Root Translation: ${conv.hasRootTranslation?"vorhanden, v0.6.5 spielt bewusst in-place":"keine"}
...
function sammyOpenPanel(id){
 ...
 if(before==="sammyAnimPanel"&&id!=="sammyAnimPanel")sammyRestorePreAnimationCamera();
 if(id==="sammyAnimPanel"&&before!=="sammyAnimPanel"&&!sammyIntroActive){
  sammyPreAnimationCamera=sammyCaptureCameraState();
 }
...
function sammyCameraTargets(mode="edit"){
 ...
 // The user calibrated these presets on the canonical default body. Only the
 // vertical world coordinates scale with body height; X/Z and the actual pan
 // (OrbitControls target) remain the calibrated values.
...
function updateDecision(){
 if(shapePass&&rigPass&&posePass){
  ...
  if(expanded&&shapeEngine==="anny"&&annyPackLoaded){
   ...
   info("#decisionInfo",`✓ v0.6.5: Anny ersetzt nur die Identity-/Rest-Shape-Quelle. Das gerenderte Low-LOD bleibt kanonische SOMA-Topologie und läuft danach durch denselben bereits getesteten shape-adaptiven 122-Joint-LBS-Pfad.
...
// Sammy v0.6.5: production shell + automatic runtime.
sammyInitUi();
