...
// v0.6.4: exact browser-side Anny blendshape engine on canonical SOMA topology.
...
function parseNPY(u8){
 ...
 } else throw new Error("NPY dtype "+descr+" noch nicht unterstützt (v0.6.4 unterstützt f4/f8, i1/i2/i4/i8, u1/u2/u4 sowie U/S-Strings)");
...
async function loadCurrentRigPack(){
 try{
  ...
  // v0.6.4: the fresh v2 rig-pack stores REAL newlines, while the very first
  ...
  if(midMissing.length){if(asset.cacheHit)await assetCacheDelete(ASSET_KEY.currentRig);throw new Error("Rig-Pack ist noch v1/Low-only. Für v0.6.4 bitte den neuen ‘Build Anny SOMA Engine v3’-Workflow einmal ausführen; danach erneut laden.")}
...
Der aktuelle v0.2.x-Rig-Datenstand ist als kleiner Browser-Pack vorhanden. v0.6.4 kann daraus jetzt direkt den internen Expanded-/Twist-Pfad mit ${targetNames.length} Skinning-Joints aktivieren; die Bedienung bleibt bei den 77 öffentlichen Pose-Joints.`);
...
v0.6.4 versucht den Rig-Pack in dieser Reihenfolge:
...
// v0.6.4: exact structural/orientation contract extracted from the uploaded
...
// v0.6.4 semantic hand-chain correction.
...
function buildSammyMixamoBridgeScene(){
 ...
 // shoulders, arms and hands. v0.6.4 bakes the official SOMA T-pose into the
 ...
 // v0.6.4: dedicated thumb-plane refinement. The general shortest-arc frame
...
async function exportMixamoComparisonFrame(){
 ...
 try{
  ...
  const bytes=new mod.FBXExporter().parseSync(bridge.scene,{axisUp:"Y",axisForward:"-Z",unitScale:100,bakeSpaceTransform:false,includeAnimations:false,customProperties:true,creator:"Sammy Axis16 Retarget Comparison v0.6.4"});
...
async function exportSammyMixamoBridge(){
 ...
 try{
  ...
  const bytes=new mod.FBXExporter().parseSync(bridge.scene,{
   ...
   creator:"Sammy Mixamo XBotContract65 T-Pose Axis16 v0.6.4"
  });
...
Mesh und Skeleton sind in die offizielle SOMA T-Pose gebacken. v0.6.4: 52-Bone-Frame-Transport + Thumb-Plane-Fix + korrigierte SOMA→Mixamo-Fingersemantik (Metacarpal/MCP/Terminal).
...
v0.6.4 korrigiert genau den im iPhone-Test sichtbaren Grundhaltungsfehler:
...
function applyAnnyAxis16RetargetPose(rest,relative3,markMoved=true,report=true,label="Axis16 → exact Anny/SOMA"){
 ...
 // v0.6.4: world motion delta is applied to the TRANSPORTED NATIVE ANNY frame.
...
async function setDisplayLOD(lod){
 if(lod===displayLOD)return true;if(lod==="mid"){
  if(shapeEngine!=="anny"){info("#lodInfo","Mid ist in v0.6.4 bewusst für den Anny-Pfad aktiviert. Zuerst Anny verwenden.");return false}
...
async function startFullShapeAnalysis(){
 ...
 try{
  if(shapeEngine!=="soma-pca")throw new Error("Der alte 128-PC-Analyzer gilt nur für SOMA-PCA. Für v0.6.4 Anny direkt über die nativen Parameter testen.");
...
Wichtig: Umfang/Tiefe sind in v0.6.4 bewusst sichtbare Slice-Proxies. Die Mathematik des Modifiers wird damit real getestet; die endgültigen BODY-LAB-Messdefinitionen werden später gegen echte anthropometrische Landmarken/Messregeln validiert.`);
  updateDecision()
 }catch(e){
  console.error(e);
...
async function testAxisReferenceFixFromRepo(){
 try{
  ...
  stopPoseAnimation(false);
  const result=applyAnnyAxis16RetargetPose(currentRestMid,rel,true,false,"v0.6.4 Axis16-Referenz-FK");
...
v0.6.4 ändert erstmals NICHT die Animation und NICHT Anny-FK selbst, sondern den dazwischenliegenden Referenzvertrag:
...
Die statische Axis16-Bridge ist ab v0.6.4 der Bewegungs-Nullpunkt. Der animierte Mixamo-T-Pose-Clip wird nur noch als Kalibrier-/Kompatibilitätsprüfung gespeichert, damit Mixamos eigene Wrist-/Thumb-Pose nicht versehentlich aus jeder Animation herausgerechnet wird.`)
}
...
async function convertMixamoFbxMotion(arrayBuffer,filename="Mixamo FBX"){
 ...
 for(const [k,b] of bones){
  // v0.6.4: motion zero is the STATIC Axis16 bridge, not Mixamo's animated
...
Motion-Zero: STATISCHE Axis16-Bridge (v0.6.4)
...
`:""}Playback: ${userAnimFps} fps · Root Translation: ${conv.hasRootTranslation?"vorhanden, v0.6.4 spielt bewusst in-place":"keine"}
...
function sammyBubbleGroupMembers(id){
 const meta=sammyBubbleRegistry[id];if(!meta?.groupId)return [id];
 return Object.keys(sammyBubbleRegistry).filter(k=>sammyBubbleVisible(k)&&sammyBubbleRegistry[k]?.groupId===meta.groupId)
}
...
function sammyPanelCameraModeFor(id){return id==="sammyAnimPanel"?"anim-panel":"edit"}
function sammyRefreshPanelCamera(){
 if(sammyIntroActive)return;
 const open=document.querySelector('.sammyPanel.open');
 sammyCameraTo(open?sammyPanelCameraModeFor(open.id):"edit",820,false)
}
...
function sammyInstallBubbleDrag(el,defaultX,defaultY){
 ...
 el.addEventListener("pointerdown",e=>{
  ...
  groupIds=sammyBubbleGroupMembers(el.id);
 ...
 el.addEventListener("pointermove",e=>{
  ...
  const cl=sammyBubbleClampPos(ox+dx,oy+dy);meta.x=cl.x;meta.y=cl.y;meta.edge=null;meta.groupId=null;sammyBubbleApply(el.id)
 });
 el.addEventListener("pointerup",e=>{
  if(pid!==e.pointerId)return;pid=null;
  const meta=sammyBubbleRegistry[el.id];
  meta.edge=sammyNearestEdgeFromPos(meta.x,meta.y);
 ...
 window.addEventListener("resize",()=>{sammyResolveBubbleLayout()})
}
...
function sammySyncAnimationUi(){
 const status=$("#sammyAnimStatus"),range=$("#sammyFrameRange"),num=$("#sammyFrameNumber"),out=$("#sammyFrameOut"),pause=$("#sammyPauseResume");
 if(!status)return;
 if(!userAnimLoaded){
  status.textContent="Keine Animation geladen";status.className="sammyStatus";
  for(const x of [range,num,pause])if(x)x.disabled=true;
...
function sammyRuntimeSnapshot(){
 return {
  app:"Sammy",version:"0.6.3",time:new Date().toISOString(),
...
function sammyCameraSnapshot(){
 const p=cam.position,r=cam.rotation,t=orbit.target;
...
function sammyCameraTargets(mode="edit"){
 ...
 const b=geometry.boundingBox,c=new THREE.Vector3(),s=new THREE.Vector3();
 b.getCenter(c);b.getSize(s);
 const fromPolar=(target,radius,pitchDeg,yawDeg)=>{
  const pitch=THREE.MathUtils.degToRad(pitchDeg),yaw=THREE.MathUtils.degToRad(yawDeg);
 ...
 if(mode==="greeting"){
  const target=new THREE.Vector3(c.x,b.min.y+s.y*0.58,c.z);
  const pos=new THREE.Vector3(c.x,target.y+s.y*0.539,c.z+s.y*1.413);
  return {target,pos}
 }
 if(mode==="anim-panel"){
  const target=new THREE.Vector3(c.x+s.x*0.15,b.min.y+s.y*0.384,c.z+s.z*0.26);
  const pos=fromPolar(target,s.y*1.992,19.76,14.01);
  return {target,pos}
 }
 const target=new THREE.Vector3(c.x,b.min.y+s.y*0.50,c.z+s.z*0.03);
 const pos=new THREE.Vector3(c.x,target.y+s.y*0.0283,c.z+s.y*0.946);
 return {target,pos}
}
function sammyCameraTo(mode="edit",duration=850,instant=false){
 const t=sammyCameraTargets(mode);if(!t)return;
 if(instant){cam.position.copy(t.pos);orbit.target.copy(t.target);orbit.update();sammyCameraTween=null;return}
 sammyCameraTween={start:performance.now(),duration,p0:cam.position.clone(),p1:t.pos.clone(),t0:orbit.target.clone(),t1:t.target.clone()}
}
function sammyUpdateCameraTween(now){
 ...
 cam.position.lerpVectors(tw.p0,tw.p1,t);orbit.target.lerpVectors(tw.t0,tw.t1,t);if(t>=1)sammyCameraTween=null
}
...
async function sammyLoadGreeting(){
 const res=await fetch("./standing-greeting.fbx?v=0.6.3",{cache:"force-cache"});
...
function sammyInitUi(){
 ...
 sammyInstallBubbleDrag($("#sammyShapeBubble"),w-66,Math.max(160,h*.43));
 // error bubble starts hidden but still receives a persisted/default position
 sammyInstallBubbleDrag($("#sammyErrorBubble"),8,Math.max(220,h*.58));
...
function updateDecision(){
 if(shapePass&&rigPass&&posePass){
  ...
  if(expanded&&shapeEngine==="anny"&&annyPackLoaded){
   ...
   info("#decisionInfo",`✓ v0.6.4: Anny ersetzt nur die Identity-/Rest-Shape-Quelle. Das gerenderte Low-LOD bleibt kanonische SOMA-Topologie und läuft danach durch denselben bereits getesteten shape-adaptiven 122-Joint-LBS-Pfad.
...
// Sammy v0.6.4: production shell + automatic runtime.
sammyInitUi();
