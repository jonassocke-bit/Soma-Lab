...
// v0.6.3: exact browser-side Anny blendshape engine on canonical SOMA topology.
...
function parseNPY(u8){
 ...
 } else throw new Error("NPY dtype "+descr+" noch nicht unterstützt (v0.6.3 unterstützt f4/f8, i1/i2/i4/i8, u1/u2/u4 sowie U/S-Strings)");
...
async function loadCurrentRigPack(){
 try{
  ...
  // v0.6.3: the fresh v2 rig-pack stores REAL newlines, while the very first
  ...
  if(midMissing.length){if(asset.cacheHit)await assetCacheDelete(ASSET_KEY.currentRig);throw new Error("Rig-Pack ist noch v1/Low-only. Für v0.6.3 bitte den neuen ‘Build Anny SOMA Engine v3’-Workflow einmal ausführen; danach erneut laden.")}
...
Der aktuelle v0.2.x-Rig-Datenstand ist als kleiner Browser-Pack vorhanden. v0.6.3 kann daraus jetzt direkt den internen Expanded-/Twist-Pfad mit ${targetNames.length} Skinning-Joints aktivieren; die Bedienung bleibt bei den 77 öffentlichen Pose-Joints.`);
...
v0.6.3 versucht den Rig-Pack in dieser Reihenfolge:
...
// v0.6.3: exact structural/orientation contract extracted from the uploaded
...
// v0.6.3 semantic hand-chain correction.
...
function buildSammyMixamoBridgeScene(){
 ...
 // shoulders, arms and hands. v0.6.3 bakes the official SOMA T-pose into the
 ...
 // v0.6.3: dedicated thumb-plane refinement. The general shortest-arc frame
...
async function exportMixamoComparisonFrame(){
 ...
 try{
  ...
  const bytes=new mod.FBXExporter().parseSync(bridge.scene,{axisUp:"Y",axisForward:"-Z",unitScale:100,bakeSpaceTransform:false,includeAnimations:false,customProperties:true,creator:"Sammy Axis16 Retarget Comparison v0.6.3"});
...
async function exportSammyMixamoBridge(){
 ...
 try{
  ...
  const bytes=new mod.FBXExporter().parseSync(bridge.scene,{
   ...
   creator:"Sammy Mixamo XBotContract65 T-Pose Axis16 v0.6.3"
  });
...
Mesh und Skeleton sind in die offizielle SOMA T-Pose gebacken. v0.6.3: 52-Bone-Frame-Transport + Thumb-Plane-Fix + korrigierte SOMA→Mixamo-Fingersemantik (Metacarpal/MCP/Terminal).
...
v0.6.3 korrigiert genau den im iPhone-Test sichtbaren Grundhaltungsfehler:
...
function applyAnnyAxis16RetargetPose(rest,relative3,markMoved=true,report=true,label="Axis16 → exact Anny/SOMA"){
 ...
 // v0.6.3: world motion delta is applied to the TRANSPORTED NATIVE ANNY frame.
...
async function setDisplayLOD(lod){
 if(lod===displayLOD)return true;if(lod==="mid"){
  if(shapeEngine!=="anny"){info("#lodInfo","Mid ist in v0.6.3 bewusst für den Anny-Pfad aktiviert. Zuerst Anny verwenden.");return false}
...
async function startFullShapeAnalysis(){
 ...
 try{
  if(shapeEngine!=="soma-pca")throw new Error("Der alte 128-PC-Analyzer gilt nur für SOMA-PCA. Für v0.6.3 Anny direkt über die nativen Parameter testen.");
...
Wichtig: Umfang/Tiefe sind in v0.6.3 bewusst sichtbare Slice-Proxies. Die Mathematik des Modifiers wird damit real getestet; die endgültigen BODY-LAB-Messdefinitionen werden später gegen echte anthropometrische Landmarken/Messregeln validiert.`);
  updateDecision()
 }catch(e){
  console.error(e);
...
async function testAxisReferenceFixFromRepo(){
 try{
  ...
  stopPoseAnimation(false);
  const result=applyAnnyAxis16RetargetPose(currentRestMid,rel,true,false,"v0.6.3 Axis16-Referenz-FK");
...
v0.6.3 ändert erstmals NICHT die Animation und NICHT Anny-FK selbst, sondern den dazwischenliegenden Referenzvertrag:
...
Die statische Axis16-Bridge ist ab v0.6.3 der Bewegungs-Nullpunkt. Der animierte Mixamo-T-Pose-Clip wird nur noch als Kalibrier-/Kompatibilitätsprüfung gespeichert, damit Mixamos eigene Wrist-/Thumb-Pose nicht versehentlich aus jeder Animation herausgerechnet wird.`)
}
...
async function convertMixamoFbxMotion(arrayBuffer,filename="Mixamo FBX"){
 ...
 for(const [k,b] of bones){
  // v0.6.3: motion zero is the STATIC Axis16 bridge, not Mixamo's animated
...
Motion-Zero: STATISCHE Axis16-Bridge (v0.6.3)
...
`:""}Playback: ${userAnimFps} fps · Root Translation: ${conv.hasRootTranslation?"vorhanden, v0.6.3 spielt bewusst in-place":"keine"}
...
function bootStatus(stage,detail="",cls="warn"){
 ...
 const toast=$("#sammyBootToast"),title=$("#sammyBootTitle"),body=$("#sammyBootDetail"),splash=$("#sammySplash");
 if(!toast)return;
 ...
 const splashActive=splash&&!splash.classList.contains("gone");
 toast.classList.remove("done","error");
 if(splashActive&&cls!=="bad"){
  toast.classList.remove("visible");
 }else{
  toast.classList.add("visible");
  if(cls==="bad")toast.classList.add("error");
 }
 if(cls==="ok"){
  sammyBootHideTimer=setTimeout(()=>{toast.classList.add("done");toast.classList.remove("visible")},1200)
 }
}
...
const SAMMY_UI_KEY="sammy-v060-ui";
...
function sammyInstallBubbleDrag(el,defaultX,defaultY){
 ...
 let x=Number(saved?.x??defaultX),y=Number(saved?.y??defaultY);
 const apply=()=>{el.style.left=`${x}px`;el.style.top=`${y}px`;el.style.right="auto";el.style.bottom="auto"};
 const clampNow=()=>{
  const r=el.getBoundingClientRect(),pad=7;
  x=Math.max(pad,Math.min(innerWidth-r.width-pad,x));
  y=Math.max(pad+Number(getComputedStyle(document.documentElement).getPropertyValue("--safe-top")||0),Math.min(innerHeight-r.height-pad,y))
 };
 clampNow();apply();
 let sx=0,sy=0,ox=0,oy=0,moved=false,pid=null;
 el.addEventListener("pointerdown",e=>{
  pid=e.pointerId;el.setPointerCapture(pid);sx=e.clientX;sy=e.clientY;ox=x;oy=y;moved=false;e.preventDefault()
 });
 el.addEventListener("pointermove",e=>{
  if(pid!==e.pointerId)return;
  const dx=e.clientX-sx,dy=e.clientY-sy;if(Math.hypot(dx,dy)>5)moved=true;
  x=ox+dx;y=oy+dy;clampNow();apply()
 });
 el.addEventListener("pointerup",e=>{
  ...
  if(edge==="left")x=pad;
  else if(edge==="right")x=innerWidth-r.width-pad;
  else if(edge==="top")y=pad+Math.max(0,parseFloat(getComputedStyle(document.body).paddingTop)||0);
  else y=innerHeight-r.height-pad;
  clampNow();apply();
  const st=sammyUiLoadState(),b={...(st.bubbles||{}),[el.id]:{x,y}};sammyUiSaveState({bubbles:b});
  if(!moved){
 ...
 window.addEventListener("resize",()=>{clampNow();apply()})
}
...
function updateDecision(){
 if(shapePass&&rigPass&&posePass){
  ...
  if(expanded&&shapeEngine==="anny"&&annyPackLoaded){
   ...
   info("#decisionInfo",`✓ v0.6.3: Anny ersetzt nur die Identity-/Rest-Shape-Quelle. Das gerenderte Low-LOD bleibt kanonische SOMA-Topologie und läuft danach durch denselben bereits getesteten shape-adaptiven 122-Joint-LBS-Pfad.
...
// Sammy v0.6.3: minimal production shell + automatic runtime.
sammyInitUi();
