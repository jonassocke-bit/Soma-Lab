...
## v0.8.19.1 · ANSUR Protocol Lab reference assets
...
## v0.8.19.1 · User-provided pose-reference FBX files

`sammy-ansur-standing-source.fbx` and `sammy-ansur-sitting-source.fbx` are copies of the two FBX motion files supplied by the project user in this conversation. They are retained only as user-provided visual/reference assets. v0.8.19.1 PROT does not load or retarget either FBX. The ANSUR posture requirements come from NATICK/TR-11/017 and the runtime pose is generated on Sammy’s native SOMA/Axis16 reference.
