...
def main():
 ...
 target_W_mid=target["weights"].astype(np.float32);public_W_mid=public["weights"].astype(np.float32)
 ...
 tmd,tmi,tmp,tms=sparse_parts(target_W_mid);pmd,pmi,pmp,pms=sparse_parts(public_W_mid)
 ...
 source=git_sha(root);arrays=dict(schema_version=b("soma-browser-current-rig-pack-v2"),source_git_sha=b(source),template_rig_sha256=b(sha256(rig_path)),lod_mid_to_low=lod.astype(np.int32),facial_excluded_low=facial,procedural_json_utf8=np.frombuffer(proc_path.read_bytes(),dtype=np.uint8).copy(),public_target_indices=public_target,target_to_public_indices=np.asarray(t2p,dtype=np.int32),target_joint_names_utf8=b("\n".join(target_names)),target_joint_parent_ids=target["joint_parent_ids"].astype(np.int32),target_bind_pose_world=target["bind_pose_world"].astype(np.float32),target_bind_pose_local=target["bind_pose_local"].astype(np.float32),target_t_pose_world=target["t_pose_world"].astype(np.float32),target_t_pose_local=target["t_pose_local"].astype(np.float32),target_bind_shape_low=target_bind_low,target_skinning_data=td,target_skinning_indices=ti,target_skinning_indptr=tp,target_skinning_shape=ts,target_skinning_mid_data=tmd,target_skinning_mid_indices=tmi,target_skinning_mid_indptr=tmp,target_skinning_mid_shape=tms,public_joint_names_utf8=b("\n".join(public_names)),public_joint_parent_ids=public["joint_parent_ids"].astype(np.int32),public_bind_pose_world=public["bind_pose_world"].astype(np.float32),public_bind_pose_local=public["bind_pose_local"].astype(np.float32),public_t_pose_world=public["t_pose_world"].astype(np.float32),public_t_pose_local=public["t_pose_local"].astype(np.float32),public_bind_shape_low=public_bind_low,public_skinning_data=pd,public_skinning_indices=pi,public_skinning_indptr=pp,public_skinning_shape=ps,public_skinning_mid_data=pmd,public_skinning_mid_indices=pmi,public_skinning_mid_indptr=pmp,public_skinning_mid_shape=pms,public_rbf_crow_indices=rbf_crow,public_rbf_col_indices=rbf_col,public_rbf_values=rbf_val,public_rbf_shape=np.asarray([len(public_names),len(lod)],dtype=np.int32))
 np.savez_compressed(out,**arrays)
 meta=dict(schema="soma-browser-current-rig-pack-v2",source_git_sha=source,template_rig_sha256=sha256(rig_path),template_rig_bytes=rig_path.stat().st_size,skin_mesh_name=target["skin_mesh_name"],target_joint_count=len(target_names),public_joint_count=len(public_names),low_vertex_count=len(lod),mid_vertex_count=int(target_W_mid.shape[0]),rbf_nnz=len(rbf_val),pack_bytes=out.stat().st_size,pack_sha256=sha256(out))
