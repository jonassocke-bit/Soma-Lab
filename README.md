...
## v0.5.17 – Current Expanded 122-Joint LBS

Nach dem realen iPhone-Test von v0.2.0 ist der kompakte v0026-Rig-Pack bewiesen. v0.5.17 aktiviert nun den eigentlichen Expanded-Skinning-Pfad:
...
Noch bewusst offen: das vollständige shape-adaptive **Rotation-Fitting** der offiziellen `SkeletonTransfer`-Pipeline. v0.5.17 verwendet für die Shape-Anpassung die offiziellen vorberechneten RBF-Jointpositionen und die aktuelle Procedural-Expansion, aber noch keine vollständige Browser-Portierung des Kabsch/Newton-Schulz Rotations-Fits.

- v0.5.17 merkt die zuletzt tatsächlich angewendete 78-Joint-Relativpose. Shape-Regler können dadurch eine laufende/aktuelle Pose nach dem Rebind wieder anwenden, statt beim Morphen ungewollt auf die Slider-Nullpose zurückzuspringen.


## v0.5.17 – 122-Joint-Hierarchie reihenfolgeunabhängig
...
Der vorhandene `soma_current_rig_pack_v0026.npz` und dessen persistenter Cache bleiben unverändert gültig; der GitHub-Actions-Builder muss für v0.5.17 nicht erneut ausgeführt werden.


## v0.5.17 – robuster Current-Rig-Pack Loader
...
## v0.5.17 – Joint-Name-Separator-Fix
...
## v0.5.17 – Shape-Space Analyzer + semantische Live-Modifier
...
sind in v0.5.17 aber bewusst sichtbare, rig-relative horizontale Slice-Proxies.
...
## v0.5.17 – Anny on top of the proven SOMA architecture
...
is intentionally **discrete**: there is no male/female mixing in the v0.5.17 UI.
...
## v0.5.17 – exact Anny engine + Low/Mid + all modifiers
...
## v0.5.17 – Rig-Pack joint-name decoder fix
...
v0.5.17 uses the existing compatibility decoder `decodePackedJointNames()`, which
supports both:
...
## v0.5.17 – automatic runtime, origin fix, motion import
...
## v0.5.17 – Safari SOMA NPZ parser regression fix
...
v0.5.17 fixes the dtype and NUL regexes and keeps the v0.5.2 automatic startup,
...
## v0.5.17 – direct Mixamo Bridge FBX export in Safari
...
## v0.5.17 – simplified Mixamo proxy rig
...
## v0.5.17 – exact Mixamo X Bot skeleton contract
...
v0.5.17 embeds only the extracted 65-bone structural/orientation contract — **not**
...
## v0.5.17 – direct Mixamo FBX import / retarget
...
## v0.5.17 – reject incompatible old Mixamo bridge files
...
v0.5.17 now hard-validates Mixamo FBX files:
...
## v0.5.17 – bridge is now actually bound in official SOMA T-pose
...
v0.5.17 changes the bridge construction itself:
...
## v0.5.17 – Mixamo T-pose reference now samples the animation clip
...
clip, so it was mathematically the wrong zero pose. v0.5.17 evaluates frame 0
...
## v0.5.17 – accept Mixamo's legitimate 54-bone motion subset
...
## v0.5.17 – thumb axis + neck follow corrections
...
v0.5.17 changes both directions:
...
A new Mixamo roundtrip should be generated from the v0.5.17 bridge because the
thumb bind axes and neck skin weights are part of the exported character.


## v0.5.17 – full 65-bone connection/axis audit
...
## v0.5.17 – targeted thumb plane refinement
...
v0.5.17 therefore keeps the general 52-bone XBot→SOMA frame transport and adds
...
## v0.5.17 – semantic hand/finger chain correction
...
v0.5.17 remaps Index/Middle/Ring/Pinky as:
...
## v0.5.17 – Axis16 import calibration hardening
...
v0.5.17 therefore freezes the export side at Axis16 and focuses on import:
