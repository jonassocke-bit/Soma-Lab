...
the candidate pose had already been computed and rendered. The candidate was
still catastrophically wrong.

The deeper invariant violation was then identified:

- Mixamo/Axis16 absolute bone frames were copied directly into the target pose.
- Skinning was still computed as `final_pose * inverse(native_Anny_rest)`.
- Therefore two different bone coordinate-frame conventions were multiplied
...
2. For each Anny bone, compute a WORLD-SPACE transport rotation from its native
   child direction(s) to the target child direction(s).
   - one child: shortest-arc rotation
   - multiple children: special orthogonal Procrustes fit
   - leaf: inherit parent transport
3. Apply that transport to the NATIVE Anny rest orientation.
4. Apply the already-proven Mixamo/Public78 world motion delta to this
   transported Anny frame.
5. FK translations use the same transported reference skeleton.
6. Skinning still uses inverse(native Anny rest), now with a compatible bone
   basis by construction.

Also fixes the UI-only typo `frameMesh()` → `frame()` that caused the v0.5.27
one-click test to show FEHLER after rendering.
